...
export class IrnBrume {
  ...
  async trySubscribe(topic: string): Promise<Result<void, Error>> {
    return await Result.unthrow(async t => {
      const client = this.pool.tryGetSync(0).throw(t).throw(t).inner.inner
  ...
  async tryClose(reason: unknown): Promise<Result<void, Error>> {
    return Result.unthrow(async t => {
      ...
      const irn = this.pool.tryGetSync(0).flatten().ok().mapSync(x => x.inner.inner)
...
export class IrnSockets {
  ...
  async trySubscribe(topic: string): Promise<Result<void, Error>> {
    return await Result.unthrow(async t => {
      const client = this.pool.inner.tryGetSync(0).throw(t).throw(t).inner.inner
      await client.trySubscribe(topic).then(r => r.throw(t))
      this.topics.add(topic)
      return Ok.void()
    })
  ...
  async tryClose(reason: unknown): Promise<Result<void, Error>> {
    return Result.unthrow(async t => {
      ...
      const irn = this.pool.inner.tryGetSync(0).flatten().ok().mapSync(x => x.inner.inner)
