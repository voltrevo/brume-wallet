          ...
          return await (async () => {
            ...
            using circuit = await loopOrThrow(async () => {
              try {
                return await (async () => {
                  start = Date.now()
                  using circuit = new Box(await tor.createOrThrow(AbortSignal.timeout(ping.value * 5)))
                  ...
                  await loopOrThrow(async () => {
                    ...
                    start = Date.now()
                    await Retry.run(() => circuit.getOrThrow().extendOrThrow(body, AbortSignal.timeout(ping.value * 5)))
