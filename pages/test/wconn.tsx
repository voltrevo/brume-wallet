...
export namespace Iridium {
  ...
  export async function trySubscribe(socket: WebSocket, topic: string): Promise<Result<void, Error>> {
    return await Result.unthrow(async t => {
      const subscription = await SafeRpc.tryRequest<string>(socket, {
      ...
      socket.addEventListener("message", (event: MessageEvent<unknown>) => {
        ...
        if (!("method" in json))
          return
        if (json.method !== "irn_subscription")
          return
        const { id, data } = (json as RpcRequestPreinit<Data>).params
        if (id !== subscription)
          return
        if (data.topic !== topic)
          return
...
export default function Page() {
  ...
  const onClick = useCallback(async () => {
    ...
    await Iridium.trySubscribe(socket, topic).then(r => r.unwrap())
  }, [url])
