...
export namespace Consensus {
  ...
  export namespace Microdesc {
    ...
    export namespace All {
      ...
      export type D = Echalote.Consensus.Microdesc.Head[]
      export type F = Error
      ...
      export function create(maybeTor: Nullable<TorClientDuplex>, storage: QueryStorage) {
        const fetcher = async (_: K, init: RequestInit) => {
          try {
            ...
            const consensus = await Echalote.Consensus.fetchOrThrow(circuit, subsignal2)
    ...
    export type K = string
    export type D = Echalote.Consensus.Microdesc
    export type F = Error
    ...
    export function create(identity: string, maybeIndex: Nullable<number>, maybeHead: Nullable<Echalote.Consensus.Microdesc.Head>, maybeCircuit: Nullable<Circuit>, storage: QueryStorage) {
      const fetcher = async (_: K, init: RequestInit) => {
        try {
          const signal = AbortSignals.getOrNever(init.signal)
          ...
          const microdesc = await Echalote.Consensus.Microdesc.fetchOrThrow(circuit, head, subsignal)
