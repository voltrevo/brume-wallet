...
import { UnauthorizedError } from "@/libs/rpc/mods/errors";
...
import { BlobbyQuery, BlobbyRef } from "../../universal/entities/blobbys";
import { OriginData, OriginQuery, PreOriginData } from "../../universal/entities/origins";
import { SeedData, SeedQuery } from "../../universal/entities/seeds";
import { SettingsQuery } from "../../universal/entities/settings";
...
export class Global {
  ...
  async setCurrentUserOrThrow(uuid: Nullable<string>, password: Nullable<string>): Promise<Nullable<UserSession>> {
    ...
    await currentUserQuery.mutateOrThrow(() => new Some(new Data(user)))
  ...
  async requestNoPopupOrThrow<T>(request: AppRequestData): Promise<RpcResponse<T>> {
    ...
    try {
      ...
      await requestQuery.deleteOrThrow()
      done.resolve()
    }
  }
    ...
    return await this.mutex.runOrWait(async slot => {
      ...
      if (sessionByOriginState.data != null) {
        ...
        script.events.on("close", async () => {
          ...
          const { id } = sessionData
          await Status.schema(id).deleteOrThrow().catch(console.warn)
        })
  ...
  async eth_accounts(context: BgEthereumContext, session: SessionData, request: RpcRequestPreinit<unknown>): Promise<string[]> {
    const user = Option.wrap(this.#user).getOrThrow()
  ...
  async eth_coinbase(context: BgEthereumContext, session: SessionData, request: RpcRequestPreinit<unknown>): Promise<Nullable<string>> {
    const user = Option.wrap(this.#user).getOrThrow()
  ...
  async eth_chainId(context: BgEthereumContext, session: SessionData, request: RpcRequestPreinit<unknown>): Promise<string> {
    if (session.type === "wc")
      throw new Error("Unexpected WalletConnect session")
  ...
  async net_version(context: BgEthereumContext, session: SessionData, request: RpcRequestPreinit<unknown>): Promise<string> {
    if (session.type === "wc")
      throw new Error("Unexpected WalletConnect session")
  ...
  async wallet_requestPermissions(context: BgEthereumContext, session: SessionData, request: RpcRequestPreinit<unknown>): Promise<RequestedPermission[]> {
  ...
  async wallet_getPermissions(context: BgEthereumContext, session: SessionData, request: RpcRequestPreinit<unknown>): Promise<Permission[]> {
  ...
  async eth_sendTransaction(context: BgEthereumContext, session: SessionData, request: RpcRequestPreinit<unknown>, mouse?: Mouse): Promise<string> {
    const user = Option.wrap(this.#user).getOrThrow()
    ...
    const chainId = ZeroHexAsInteger.fromOrThrow(context.chain.chainId)
  ...
  async personal_sign(context: BgEthereumContext, session: SessionData, request: RpcRequestPreinit<unknown>, mouse?: Mouse): Promise<string> {
    const user = Option.wrap(this.#user).getOrThrow()
  ...
  async eth_signTypedData_v4(context: BgEthereumContext, session: SessionData, request: RpcRequestPreinit<unknown>, mouse?: Mouse): Promise<string> {
    const user = Option.wrap(this.#user).getOrThrow()
  ...
  async wallet_switchEthereumChain(context: BgEthereumContext, session: SessionData, request: RpcRequestPreinit<unknown>, mouse: Mouse): Promise<void> {
    const user = Option.wrap(this.#user).getOrThrow()
    ...
    await sessionQuery.mutateOrThrow(() => new Some(new Data({ ...sessionData, chain })))
  ...
  async brume_createUser(foreground: RpcRouter, request: RpcRequestPreinit<unknown>): Promise<void> {
    ...
    await userQuery.mutateOrThrow(() => new Some(new Data(userData)))
  }
  ...
  async brume_disconnect(foreground: RpcRouter, request: RpcRequestPreinit<unknown>): Promise<void> {
    ...
    await sessionQuery.deleteOrThrow()
  ...
  async brume_eth_fetch(foreground: RpcRouter, request: RpcRequestPreinit<unknown>): Promise<unknown> {
    ...
    const [uuid, chainId, subrequest] = (request as RpcRequestPreinit<[string, number, EthereumChainlessRpcRequestPreinit<unknown>]>).params
  ...
  async routeCustomOrThrow(context: BgEthereumContext, request: EthereumChainlessRpcRequestPreinit<unknown>, storage: QueryStorage): Promise<SimpleQuery<any, any, Error>> {
    ...
    if (request.method === BgEns.Reverse.method)
      return await BgEns.Reverse.parseOrThrow(context, request, storage)
    if (request.method === BgSimulation.method)
      return await BgSimulation.parseOrThrow(context, request, storage)
  ...
  async #wcReconnectOrThrow(sessionData: WcSessionData): Promise<WcSession> {
    ...
    if (settlement != null) {
      ...
      const sessionQuery = BgSession.schema(sessionData.id, user.storage)
      await sessionQuery.mutateOrThrow(Mutators.mapExistingData(d => d.mapSync(x => ({ ...x, settlement: undefined }))))
    }
    ...
    const onRequest = async (suprequest: RpcRequestPreinit<unknown>) => {
      ...
      if (request.method === "personal_sign")
        return new Some(await this.personal_sign(context, sessionData, request))
      if (request.method === "eth_signTypedData_v4")
        return new Some(await this.eth_signTypedData_v4(context, sessionData, request))
      return new None()
    }
