...
import { ApduConnector } from "../apdu";
...
export async function tryGetAppConfig(device: ApduConnector): Promise<Result<AppConfig, Error>> {
  return await Result.runAndDoubleWrap(() => getAppConfigOrThrow(device))
}

export async function getAppConfigOrThrow(device: ApduConnector): Promise<AppConfig> {
 ...
 * @returns 
 */
export async function tryGetAddress(device: ApduConnector, path: string): Promise<Result<GetAddressResult, Error>> {
 ...
 * @returns 
 */
export async function getAddressOrThrow(device: ApduConnector, path: string): Promise<GetAddressResult> {
 ...
 * @returns 
 */
export async function tryVerifyAndGetAddress(device: ApduConnector, path: string): Promise<Result<GetAddressResult, Error>> {
 ...
 * @returns 
 */
export async function verifyAndGetAddressOrThrow(device: ApduConnector, path: string): Promise<GetAddressResult> {
  const paths = Paths.from(path)
...
export async function trySignPersonalMessage(device: ApduConnector, path: string, message: Uint8Array): Promise<Result<SignatureInit, Error>> {
  return await Result.runAndDoubleWrap(() => signPersonalMessageOrThrow(device, path, message))
}

export async function signPersonalMessageOrThrow(device: ApduConnector, path: string, message: Uint8Array): Promise<SignatureInit> {
  const paths = Paths.from(path)
...
export async function trySignTransaction(device: ApduConnector, path: string, transaction: Transaction): Promise<Result<SignatureInit, Error>> {
  return await Result.runAndDoubleWrap(() => signTransactionOrThrow(device, path, transaction))
}

export async function signTransactionOrThrow(device: ApduConnector, path: string, transaction: Transaction): Promise<SignatureInit> {
  const paths = Paths.from(path)
...
export async function trySignEIP712HashedMessage(device: ApduConnector, path: string, domain: Uint8Array<32>, message: Uint8Array<32>): Promise<Result<SignatureInit, Error>> {
  return await Result.runAndDoubleWrap(() => signEIP712HashedMessageOrThrow(device, path, domain, message))
}

export async function signEIP712HashedMessageOrThrow(device: ApduConnector, path: string, domain: Uint8Array<32>, message: Uint8Array<32>): Promise<SignatureInit> {
  const paths = Paths.from(path)
