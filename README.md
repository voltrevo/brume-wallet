...
Use at your own risk, see [Security](#security) for more details
...
- All storage keys are hashed using HMAC-SHA256
...
(Or owning the exit node, since we don't currently check TLS certificates from the JSON-RPC server, the exit node could redirect you to its own JSON-RPC server (Fixed soon))
