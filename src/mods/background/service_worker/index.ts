...
export class Global {
  ...
  async #wcReconnectOrThrow(sessionData: WcSessionData): Promise<WcSession> {
    ...
    const sessionClient = CryptoClient.createOrThrow(irn, topic, sessionKeyBytes32, ping.value * 10)
    const session = new WcSession(sessionClient, metadata)

    await irn.subscribeOrThrow(topic, AbortSignal.timeout(ping.value * 100))
  ...
  async brume_wc_connect(foreground: RpcRouter, request: RpcRequestPreinit<unknown>): Promise<WcMetadata> {
    ...
    const [session, settlement] = await Wc.pairOrThrow(irn, pairParams, metadata, walletData.address, chains, ping.value * 10)
